...
def model_size(params: hk.Params) -> int:
...
def train_model(
    ...
    n_steps: int = 1000,
    penalty_scale: float = 1e-10,
    loss_fun: str = 'categorical',
    do_plot: bool = True,
    truncate_seq_length: Optional[int] = 200,
  ...
  def categorical_loss(
    ...
    #jax.debug.print("Inside categorical_loss")
    #jax.debug.print("Labels: {}", labels)
    #jax.debug.print("Output logits: {}", output_logits)
  ...
  def penalized_categorical_loss(
    ...
    #jax.debug.print("Inside penalized_categorical_loss")
    #jax.debug.print("Output logits: {}", output_logits)
    #jax.debug.print("Penalty term: {}", penalty)
    log_likelihood = categorical_log_likelihood(targets, output_logits)
    #jax.debug.print("Categorical Log Likelihood: {}", log_likelihood)
    total_loss = log_likelihood + penalty_scale * penalty
...
def fit_model(
  ...
  params, opt_state, _ = train_model(
      ...
      truncate_seq_length=truncate_seq_length
  )
